...
int main(int argc, char **argv)
{
...
#else
    std::chrono::monotonic_clock::time_point prev_time = std::chrono::monotonic_clock::now();
#endif

        for(int ni=0; ni<nImages[seq]; ni+=stride, proccIm+=stride)
        {
            ...
            // Use high-resolution steady/monotonic clock to get fractional seconds between frames
#ifdef COMPILEDWITHC11
            std::chrono::steady_clock::time_point curr_time = std::chrono::steady_clock::now();
            float delta = std::chrono::duration_cast<std::chrono::duration<float> >(curr_time - prev_time).count();
            prev_time = curr_time;
#else
            std::chrono::monotonic_clock::time_point curr_time = std::chrono::monotonic_clock::now();
            float delta = std::chrono::duration_cast<std::chrono::duration<float> >(curr_time - prev_time).count();
            prev_time = curr_time;
#endif
            vTimesTrack[ni] = delta;
            
            
            // Dont wait to load the next frame
            continue;
    ...
    if (bFileName)
    {
        ...
        ifstream f_file_in(f_file);
        if (!f_file_in.is_open()) {
        ...
        ofstream f_file_out(f_file);
        if (!f_file_out.is_open()) {
            cerr << "Failed to open file for writing\n";
            return 0;
        }

        size_t n = min(lines.size(), vTimesTrack.size());

        
        if (lines.size() != vTimesTrack.size()) {
            cerr << "WARNING: Trajectory lines and delta vector sizes do not match!!\n" << lines.size() << " and " << vTimesTrack.size() << "\n";
        }

        for (size_t i = 0; i < n; i++)
        {
            f_file_out << lines[i] << " " << vTimesTrack[i*stride] << '\n';
        }
    }
    else
    {
