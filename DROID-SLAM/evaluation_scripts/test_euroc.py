...
def image_stream(datapath, image_size=[320, 512], stereo=False, stride=1):
    ...
    images_right = sorted(glob.glob(os.path.join(datapath, 'mav0/cam1/data/*.png')))[::stride]
...
if __name__ == '__main__':
    ...
    deltas = []
    # run with specified stride
    using_stride = 3

    delta_prev = time.time()
    for (t, image, intrinsics) in tqdm(images[::using_stride], desc=scene):
        droid.track(t, image, intrinsics=intrinsics)
        delta_post = time.time()
        deltas.append(delta_post - delta_prev)
        delta_prev = delta_post
    ...
    track_lines = traj_est.as_text()
    
    len_lines = len(track_lines)
    len_deltas = len(deltas)
    print(f"Len lines: {len_lines}")
    print(f"Len deltas: {len_deltas}")
    with open("track_output.txt", "w") as f:
        for i, line in enumerate(track_lines):
            # write line and add deltas
            delta = deltas[i // using_stride] if i % using_stride == 0 else 0
            f.write(line + " {:.6f}\n".format(delta))

    
    # Calculate error internally

    #traj_ref = file_interface.read_tum_trajectory_file(args.gt)
    #traj_ref, traj_est = sync.associate_trajectories(traj_ref, traj_est)
    #result = main_ape.ape(traj_ref, traj_est, est_name='traj', pose_relation=PoseRelation.translation_part, align=True, correct_scale=True)
