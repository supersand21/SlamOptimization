...
class MSCKF(object):
    ...
    def feature_callback(self, feature_msg):
        ...
        #pprint('---prune_cam_state_buffer  ', time.time() - t)
    ...
    def publish(self, timestamp):
        ...
        #x_err = self.state_server.state_cov[12, 12]
        #y_err = self.state_server.state_cov[13, 13]
        #z_err = self.state_server.state_cov[14, 14]

        #print(x_err, y_err, z_err)

        line = f"{int(imu_state.timestamp * 1e+9)}{'.000000'} {imu_state.position[0]} {imu_state.position[1]} {imu_state.position[2]} {imu_state.orientation[0]} {imu_state.orientation[1]} {imu_state.orientation[2]} {imu_state.orientation[3]} {delta}"
        #print(line)
        with open("track_output.txt", 'a') as f:
            f.write(line + "\n")
        ...
        return namedtuple('vio_result', ['timestamp', 'pose', 'velocity', 'cam0_pose'])(
            timestamp, T_b_w, body_velocity, T_c_w)