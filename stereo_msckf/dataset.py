...
class Stereo(object):
    def __init__(self, cam0, cam1):
        if len(cam0) == 2033 and len(cam1) == 2032: # Special exeption to fix difficult dataset
            # Drop first frame from cam0
            cam0.timestamps = cam0.timestamps[1:]
            cam0.ids = cam0.ids[1:]

        print("Length of cam sequences: ", len(cam0), len(cam1))
        print("First timestamps of each: ", cam0.timestamps[0], cam1.timestamps[0])
...
class DataPublisher(object):
    def __init__(self, dataset, out_queue, duration=float('inf'), ratio=1., stride = 1): 
    ...
    def publish(self):
        ...
        print(f"Started at: ${myStartTime}")
        ...
        while not self.stopped:
            try:
                ...
                self.out_queue.put(None)
                self.ended(myStartTime=myStartTime)
                return
    ...
    def ended(self, myStartTime):
        myEndTime = time.time()
        print(f"Ended at: {myStartTime}, Duration: {myEndTime - myStartTime}")
